    ...
    try {
      ...
      session.on(AgentEventsEnum.USER_TRANSCRIPTION, (p) => {
        if (p?.text) setMessages(prev => {
          const c = [...prev];
          if (c.length && c[c.length-1]?.role === 'user' && !c[c.length-1]?.final) c[c.length-1] = { role: 'user', text: p.text, final: false };
          else c.push({ role: 'user', text: p.text, final: false });
          return c;
        });
      });
      session.on(AgentEventsEnum.USER_SPEAK_ENDED, () => setMessages(prev => {
        const c = [...prev]; if (c.length && c[c.length-1]?.role === 'user') c[c.length-1] = { ...c[c.length-1], final: true }; return c;
      }));
      ...
      session.on(AgentEventsEnum.AVATAR_TRANSCRIPTION, (p) => {
        if (p?.text) setMessages(prev => {
          const c = [...prev];
          if (c.length && c[c.length-1]?.role === 'assistant' && !c[c.length-1]?.final) c[c.length-1] = { role: 'assistant', text: p.text, final: false };
          else c.push({ role: 'assistant', text: p.text, final: false });
          return c;
        });
      });
      session.on(AgentEventsEnum.AVATAR_SPEAK_ENDED, () => setMessages(prev => {
        const c = [...prev]; if (c.length && c[c.length-1]?.role === 'assistant') c[c.length-1] = { ...c[c.length-1], final: true }; return c;
      }));
      ...
      await Promise.race([
        session.start(),
        new Promise((_, reject) => setTimeout(() => reject(new Error('Taking longer than usual. Tap Try Again.')), 15000))
      ]);
      sessionRef.current = session;

      // Auto-end at ~1 min
      setTimeout(() => { if (sessionRef.current) { try { session.stop(); } catch {} sessionRef.current = null; } }, 58000);
      ...
      {open && (
        <div className="fixed bottom-6 right-6 z-50 w-[420px] h-[560px] bg-gray-900 border border-gray-700 rounded-2xl shadow-2xl flex flex-col overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-cyan-600 to-blue-700 text-white shrink-0">
            ...
            <button onClick={() => { stopSession(); setMessages([]); setMode('choose'); setOpen(false); }} className="hover:bg-white/20 p-1 rounded-lg"><X size={20} /></button>
          </div>
            ...
            {mode === 'loading' && (
              <div className="flex-1 flex flex-col items-center justify-center gap-3">
                <div className="relative w-16 h-16">
                  ...
                  <div className="absolute inset-2 rounded-full bg-cyan-500/30 flex items-center justify-center">
                    <Volume2 size={24} className="text-cyan-400" />
                  </div>
                </div>
                <p className="text-white text-sm font-medium">Connecting to avatar...</p>
                <p className="text-gray-500 text-xs">This takes about 10 seconds</p>
              </div>
            )}
