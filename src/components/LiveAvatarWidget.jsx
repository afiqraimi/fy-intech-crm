...
function LiveAvatarWidget() {
  ...
  const handleOpen = () => { setOpen(true); setMode('choose'); setError(''); setEmbedUrl(''); };
      ...
      {open && (
        <div className="fixed bottom-6 right-6 z-50 w-[420px] h-[560px] bg-gray-900 border border-gray-700 rounded-2xl shadow-2xl flex flex-col overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-cyan-600 to-blue-700 text-white shrink-0">
            ...
            <button onClick={() => { setEmbedUrl(''); setMode('choose'); setOpen(false); setMessages([]); }} className="hover:bg-white/20 p-1 rounded-lg"><X size={20} /></button>
          </div>
          ...
          <div className="flex-1 flex flex-col bg-gray-950 overflow-hidden">
            ...
            {mode === 'avatar' && embedUrl && (
              <div className="flex-1 flex flex-col">
                ...
                <p className="text-gray-500 text-[10px] text-center py-1 shrink-0 bg-gray-900">Click CC icon in video for subtitles • ~1 min session</p>
              </div>
            )}
